"""
Seed script — populates the database with mock data for the demo.

  python -m scripts.seed            # DESTRUCTIVE: wipe all tables, then re-insert demo data
  python -m scripts.bootstrap       # idempotent: seed only if the DB is empty (used on deploy)

`build(db)` inserts the demo data (mock users + ~20 dummy timesheets etc.) and is shared with
scripts/bootstrap.py. Timesheet rows are generated relative to "now" at insert time, so the
demo's current-month view is populated whenever this runs.
"""
import sys
import os
...
def build(db) -> None:
    """Insert the demo dataset. Assumes the relevant tables are empty. Does not commit —
...
def reset(db) -> None:
    """Wipe all data, then re-insert the demo dataset."""
...
def seed() -> None:
    # Create tables (idempotent) so the script works standalone against an empty DB.
